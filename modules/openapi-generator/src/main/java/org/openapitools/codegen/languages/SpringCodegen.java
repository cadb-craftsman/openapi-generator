...
public class SpringCodegen extends AbstractJavaCodegen
	...
	@Setter
	protected boolean useFeignClientContextId = true;
	@Setter
	...
	@Setter
	protected boolean generateGenericResponseEntity = false;
	@Setter
	 ...
	 * {@link DocumentationProviderFeatures.DocumentationProvider} requires us to
	 * bootstrap and configure swagger-ui by ourselves. Springdoc, for example ships
	 * its own swagger-ui integration.
	 *
	 * @return true if the selected DocumentationProvider requires us to bootstrap
	 *         swagger-ui.
	 */
	...
	public void processOpts() {
		...
		// TODO remove "file" from reserved word list as feign client doesn't support
		// using `baseName`
		...
		convertPropertyToTypeAndWriteBack(REQUEST_MAPPING_OPTION, RequestMappingMode::valueOf,
				this::setRequestMappingMode);

		// Please refrain from updating values of Config Options after
		// super.ProcessOpts() is called
		super.processOpts();
		...
		convertPropertyToBooleanAndWriteBack(GENERATE_CONSTRUCTOR_WITH_REQUIRED_ARGS,
				value -> this.generatedConstructorWithRequiredArgs = value);
		...
		if (isUseSpringBoot3()) {
			if (DocumentationProvider.SPRINGFOX.equals(getDocumentationProvider())) {
				throw new IllegalArgumentException(
						DocumentationProvider.SPRINGFOX.getPropertyName() + " is not supported with Spring Boot > 3.x");
			}
			if (AnnotationLibrary.SWAGGER1.equals(getAnnotationLibrary())) {
				throw new IllegalArgumentException(
						AnnotationLibrary.SWAGGER1.getPropertyName() + " is not supported with Spring Boot > 3.x");
			}
			useJakartaEe = true;
			applyJakartaPackage();
		}
		...
		*/
		...
		// apiController: hide implementation behind undocumented flag to temporarily
		// preserve code
	...
	/*
	 * Add dynamic imports based on the parameters and vendor extensions of an
	 * operation. The imports are expanded by the mustache {{import}} tag available
	 * to model and api templates.
	 *
	 * #8315 Also handles removing 'size', 'page' and 'sort' query parameters if
	 * using 'x-spring-paginated'.
	 */
	@Override
	public CodegenOperation fromOperation(String path, String httpMethod, Operation operation, List<Server> servers) {

		// add Pageable import only if x-spring-paginated explicitly used
		// this allows to use a custom Pageable schema without importing Spring
		// Pageable.
		...
		if (codegenOperation.vendorExtensions.containsKey("x-spring-paginated")) {
			...
			List<String> defaultPageableQueryParams = new ArrayList<>(Arrays.asList("page", "size", "sort"));

			// #8315 Remove matching Spring Data Web default query params if
			// 'x-spring-paginated' with Pageable is used
			codegenOperation.queryParams.removeIf(param -> defaultPageableQueryParams.contains(param.baseName));
			codegenOperation.allParams
					.removeIf(param -> param.isQueryParam && defaultPageableQueryParams.contains(param.baseName));
		}
		...
		if (reactive) {
			...
			if (sse) {
				...
				/*
				 * expected definition: content: text/event-stream: schema: type: array format:
				 * event-stream items: type: <type> or $ref: <typeRef>
				 */
				Map<String, List<Schema>> schemaTypes = operation.getResponses().entrySet().stream()
						...
						.map(p -> p.getLeft().getContent().get(MEDIA_EVENT_STREAM)).map(MediaType::getSchema)
						.collect(Collectors.toList()).stream().collect(Collectors.groupingBy(Schema::getType));
				if (schemaTypes.containsKey("array")) {
					...
					Set<String> itemTypes = eventTypes.stream()
							.map(schema -> schema.getItems().getType() != null ? schema.getItems().getType()
	...
	public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> objs) {
		...
		for (CodegenModel cm : allModels.values()) {
			boolean hasLombokNoArgsConstructor = lombokAnnotations != null
					&& lombokAnnotations.containsKey("NoArgsConstructor");
